...
builder.Services.AddDbContext<AppDbContext>(options =>
    options.UseNpgsql(string.IsNullOrWhiteSpace(connectionString)
        ? "Host=localhost;Database=blunzinger_chess;Username=postgres;Password=postgres"
        : connectionString));
...
{
    options.Cookie.Name = "BlunzingerExtAuth";
...
.AddJwtBearer(options =>
{
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        ValidIssuer = builder.Configuration["Jwt:Issuer"] ?? "BlunzingerChess",
        ValidAudience = builder.Configuration["Jwt:Audience"] ?? "BlunzingerChess",
...
builder.Services.AddHostedService<SimulationProgressService>();
